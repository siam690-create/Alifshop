...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h5 class="fw-bold m-0">🏆 টপ সেলিং পণ্য (Top Selling Products)</h5>
        <a href="{{ route('reseller.products.index') }}" class="btn btn-outline-primary btn-sm rounded-pill px-3">সব দেখুন</a>
    </div>
    ...
    <div class="row g-4 mb-5">
        ...
        <div class="col-xl-3 col-lg-4 col-sm-6">
            <div class="product-card h-100 d-flex flex-column">
                <div class="position-relative">
                    <span class="product-badge text-warning">Top</span>
                    @if($product['image'])
                        <img src="{{ asset($product['image']) }}" class="product-img" alt="{{ $product['name'] }}">
                    @else
                        <div class="product-img d-flex align-items-center justify-content-center bg-light">
                ...
                <div class="p-3 flex-grow-1 d-flex flex-column">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-2 mb-3">
                        <span class="profit-chip">লাভ: ৳{{ number_format($product['profit'], 0) }}</span>
                        <span class="text-muted small">বিক্রি: {{ number_format($product['sold_qty'] ?? 0) }}</span>
                    </div>
                    <div class="mt-auto">
                        <div class="d-flex justify-content-between align-items-end">
                            <div>
        ...
        <div class="col-12">
            <div class="text-center py-5">
                ...
                <p class="text-muted">কোন টপ সেলিং পণ্য পাওয়া যায়নি</p>
            </div>
        </div>
        @endforelse
    </div>
                    ...
                    <table class="table align-middle mb-0 table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                @php
                                    $dashboardWalletAmount = ((string) ($order->order_status ?? '') === '11' || str_contains(strtolower((string) optional($order->status)->name), 'return'))
                                        ? -1 * (float) ($order->shipping_charge ?? 0)
                                        : ((((string) ($order->order_status ?? '') === '6') || str_contains(strtolower((string) optional($order->status)->name), 'complete') || str_contains(strtolower((string) optional($order->status)->name), 'deliver'))
                                            ? (float) ($order->reseller_profit ?? 0)
                                            : 0);
                                @endphp
                                <td class="fw-bold small {{ $dashboardWalletAmount < 0 ? 'text-danger' : ($dashboardWalletAmount > 0 ? 'text-success' : 'text-muted') }}">
                                    {{ $dashboardWalletAmount < 0 ? '-' : ($dashboardWalletAmount > 0 ? '+' : '') }} ৳{{ number_format(abs($dashboardWalletAmount), 0) }}
                                </td>
                                <td>
                                    @php
