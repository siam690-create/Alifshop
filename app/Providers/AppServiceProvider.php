            ...
            if (Auth::guard('admin')->check()) {
                ...
                if ($isReseller) {
                    $resellerData = Cache::remember('reseller_notifications_' . $resellerUser->id, 120, function () use ($resellerUser) {
                        $walletOrders = Order::whereNotNull('reseller_profit')
                        ...
                        $walletBalance = round($walletOrders->sum(function ($order) {
                            $statusName = strtolower((string) optional($order->status)->name);
                            $isReturned = str_contains($statusName, 'return');
                            $isCancelled = str_contains($statusName, 'cancel');
                            $statusId = (string) ($order->order_status ?? '');
                            $isCompleted = $statusId === '6'
                        ...
                        return compact('pendingOrdersCount', 'pendingWithdrawalsCount', 'recentOrders', 'recentWithdrawals', 'verificationStatus', 'totalNotifications', 'walletBalance');
                    });
                    ...
                    view()->share('resellerWalletBalance', $resellerData['walletBalance']);
