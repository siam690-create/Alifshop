...
class CheckCourierOrderStatus extends Command
{
     ...
     * @var string
     */
    protected $description = 'Check order status from Pathao, Steadfast, RedX, and Paperfly courier APIs and update order status automatically';
    ...
    public function handle()
    {
        ...
        $orders = Order::query()
            ...
            ->whereIn('courier_type', ['pathao', 'steadfast', 'redx', 'paperfly'])
    ...
    private function checkOrderStatus(Order $order)
    {
        ...
        } elseif ($order->courier_type === 'paperfly') {
            return $this->checkPaperflyStatus($order);
        }
    ...
    /**
     * Check Paperfly order status
     *
     * @param Order $order
     * @return int|null
     */
    private function checkPaperflyStatus(Order $order)
    {
        $paperflyInfo = Courierapi::where(['status' => 1, 'type' => 'paperfly'])->first();

        if (!$paperflyInfo || empty($paperflyInfo->username) || empty($paperflyInfo->password)) {
            Log::warning("Paperfly not configured or credentials missing");
            return null;
        }

        $referenceCandidates = array_values(array_unique(array_filter([
            trim((string) $order->courier_tracking_id),
            trim((string) $order->consignment_id),
            trim((string) $order->invoice_id),
        ])));

        if (empty($referenceCandidates)) {
            Log::warning("Paperfly reference number missing for order", ['order_id' => $order->id]);
            return null;
        }

        try {
            foreach ($referenceCandidates as $referenceNumber) {
                $response = Http::withBasicAuth($paperflyInfo->username, $paperflyInfo->password)
                    ->withHeaders([
                        'paperflykey' => (string) $paperflyInfo->api_key,
                        'Content-Type' => 'application/json',
                        'Accept' => 'application/json',
                    ])
                    ->acceptJson()
                    ->asJson()
                    ->timeout(30)
                    ->post('https://api.paperfly.com.bd/API-Order-Tracking', [
                        'ReferenceNumber' => $referenceNumber,
                    ]);

                if (!$response->successful()) {
                    Log::warning("Paperfly API error", [
                        'order_id' => $order->id,
                ...
                $data = $response->json();
                $tracking = data_get($data, 'success.trackingStatus.0');

                if (!is_array($tracking)) {
                    Log::warning("Paperfly tracking payload missing", [
                        'order_id' => $order->id,
                        'reference_number' => $referenceNumber,
                        'response' => $data,
                    ]);
                ...
                $statusMap = [
                    ...
                    'ReceivedAtPoint' => 'received at point',
                    'PickedForDelivery' => 'picked for delivery',
                    'Pick' => 'picked',
                    'onHoldSchedule' => 'on hold',
                ];

                foreach ($statusMap as $key => $mappedStatus) {
                    $value = trim((string) ($tracking[$key] ?? ''));

                    if ($value !== '' && strtolower($value) !== 'null') {
                        return $this->mapCourierStatusToOrderStatus($mappedStatus);
                    }
                }
            }
            ...
            Log::error("Paperfly status check error", [
                'order_id' => $order->id,
                'reference_numbers' => $referenceCandidates,
                'error' => $e->getMessage(),
            ]);
            throw $e;
        }
