<?php

namespace App\Http\Controllers\Reseller;

use App\Http\Controllers\Controller;
use App\Models\Order;
use App\Models\ResellerWithdrawal;
use App\Models\User;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use Toastr;

class WithdrawalController extends Controller
{
    public function index()
    {
        $user = Auth::guard('admin')->user();
        $withdrawals = ResellerWithdrawal::where('user_id', $user->id)
            ->latest()
            ->paginate(10);

        $walletBalance = round((float) ($user->wallet_balance ?? 0), 2);
        $minWithdraw = (float) config('app.reseller_min_withdraw', 100);
        $canWithdraw = $walletBalance >= $minWithdraw;

        return view('reseller.withdrawals.index', compact('user', 'withdrawals', 'walletBalance', 'minWithdraw', 'canWithdraw'));
    }

    public function store(Request $request)
    {
        Toastr::info('Manual withdraw is disabled. Your payable orders will be grouped into invoices automatically.', 'Invoice Enabled');
        return redirect()->route('reseller.invoices.index');

        $user = Auth::guard('admin')->user();
        $walletBalance = round((float) ($user->wallet_balance ?? 0), 2);
        $min = config('app.reseller_min_withdraw', 100);

        if ($walletBalance < $min) {
            return back()->with('error', 'Current balance is not enough for withdrawal request.');
        }

        $request->validate([
            'amount' => 'required|numeric|min:' . $min,
            'payout_method' => 'required|string|max:50',
            'account_name' => 'nullable|string|max:191',
            'account_number' => 'nullable|string|max:191',
            'note' => 'nullable|string|max:500',
        ]);

        $amount = round((float) $request->amount, 2);

        if ($amount > $walletBalance) {
            return back()->with('error', 'You do not have enough balance.');
        }

        DB::transaction(function () use ($user, $amount, $request) {
            // Hold balance immediately
            $user->wallet_balance -= $amount;
            $user->save();

            ResellerWithdrawal::create([
                'user_id' => $user->id,
                'amount' => $amount,
                'charge' => 0,
                'payout_method' => $request->payout_method,
                'account_name' => $request->account_name,
                'account_number' => $request->account_number,
                'note' => $request->note,
                'status' => 'pending',
            ]);
        });

        return back()->with('success', 'Withdraw request submitted successfully.');
    }
}
