...
{
    private function isReturnedResellerOrder(Order $order): bool
    {
        $statusName = strtolower((string) optional($order->status)->name);

        return str_contains($statusName, 'return');
    }

    private function isCancelledResellerOrder(Order $order): bool
    {
        $statusName = strtolower((string) optional($order->status)->name);

        return str_contains($statusName, 'cancel');
    }

    private function isCompletedResellerOrder(Order $order): bool
    {
    ...
    private function getResellerWalletImpact(Order $order): float
    {
        if ($this->isReturnedResellerOrder($order)) {
            return -1 * (float) ($order->shipping_charge ?? 0);
        }

        if ($this->isCancelledResellerOrder($order)) {
            return 0;
        }

        if ($this->isCompletedResellerOrder($order)) {
            return (float) ($order->reseller_profit ?? 0);
        }

        return 0;
    }

    private function getResellerEarnedAmount(Order $order): float
    {
        if ($this->isReturnedResellerOrder($order)) {
            return -1 * (float) ($order->shipping_charge ?? 0);
        }
    ...
    private function getResellerTransactionAmount(Order $order): float
    {
        if ($this->isReturnedResellerOrder($order)) {
            return -1 * (float) ($order->shipping_charge ?? 0);
        }
    ...
    public function index()
    {
        ...
        $totalProfit = $orders->sum(fn ($order) => $this->getResellerWalletImpact($order));
        ...
        // Hot deal products selected by admin.
        $hotDealProducts = Product::whereNotNull('reseller_price')
            ->where('status', 1)
            ...
            ->limit(8)
            ->get()
            ->map(fn ($product) => $this->formatDashboardProduct($product));

        // Best sellers based on total sold quantity.
        $topSellingProductIds = OrderDetails::query()
        ...
        $topSellingProducts = Product::whereIn('id', $topSellingProductIds)
            ...
            ->sortBy(function ($product) use ($topSellingProductIds) {
                return array_search($product->id, $topSellingProductIds->all(), true);
            })
            ->values()
            ->map(function ($product) {
                $formatted = $this->formatDashboardProduct($product);
                $formatted['sold_qty'] = (int) OrderDetails::where('product_id', $product->id)->sum('qty');
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            })->sum(fn ($order) => $this->getResellerWalletImpact($order));
    ...
    private function formatDashboardProduct(Product $product): array
    {
    ...
    public function wallet()
    {
        ...
        $allOrders->transform(function ($order) {
            $order->wallet_display_amount = $this->getResellerTransactionAmount($order);
            $order->wallet_current_impact = $this->getResellerWalletImpact($order);
            $order->wallet_earned_amount = $this->getResellerEarnedAmount($order);
            $order->is_wallet_deduction = $order->wallet_display_amount < 0;

            return $order;
        });

        $walletBalance = round($allOrders->sum('wallet_current_impact'), 2);
        $totalEarned = round($allOrders->sum('wallet_earned_amount'), 2);
        $totalOrders = $allOrders->count();

        if ((float) ($user->wallet_balance ?? 0) !== $walletBalance) {
        ...
        return view('reseller.wallet', compact('user', 'orders', 'walletBalance', 'totalEarned', 'totalOrders'));
    }
