...
class ProductCatalogController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        switch ($sort) {
            case 'price_low':
                $query->reorder()->orderBy('reseller_price', 'asc')->orderBy('id', 'desc');
                break;
            case 'price_high':
                $query->reorder()->orderBy('reseller_price', 'desc')->orderBy('id', 'desc');
                break;
            case 'name_asc':
                $query->reorder()->orderBy('name', 'asc')->orderBy('id', 'desc');
                break;
            case 'name_desc':
                $query->reorder()->orderBy('name', 'desc')->orderBy('id', 'desc');
                break;
            case 'profit_high':
                // Sort by profit (new_price - reseller_price) descending
                $query->reorder()->orderByRaw('(new_price - reseller_price) DESC')->orderBy('id', 'desc');
                break;
            default:
                $query->reorder()->orderBy('created_at', 'desc')->orderBy('id', 'desc');
        }
