...
public function sms_update(Request $request)
{
    ...
    public function courier_manage ()
    {
        ...
        $paperfly = Courierapi::where('type','=','paperfly')->first();

        if ($steadfast && empty($steadfast->token)) {
            $steadfast->token = 'SellwayBD_Steadfast_' . Str::random(24);
            $steadfast->save();
        }
        ...
        if (!$paperfly) {
            $paperfly = Courierapi::create([
                'type' => 'paperfly',
                'url' => 'https://api.paperfly.com.bd/merchant/api/service/new_order_v2.php',
                'secret_key' => 'SellwayBD_Paperfly_' . Str::random(24),
                'status' => 0,
            ]);
        }

        if (empty($paperfly->secret_key)) {
            $paperfly->secret_key = 'SellwayBD_Paperfly_' . Str::random(24);
            $paperfly->save();
        }
        
        return view('backEnd.apiintegration.courier_manage',compact('steadfast','pathao','redx','paperfly'));
    }
    ...
    public function courier_update (Request $request)
    {
        ...
        if ($update_data->type === 'paperfly') {
            unset($input['secret_key']);

            if (empty($update_data->secret_key)) {
                $input['secret_key'] = 'SellwayBD_Paperfly_' . Str::random(24);
            }
        }
        ...
        if ($update_data->type === 'steadfast') {
            ...
            if (empty($update_data->token)) {
                $input['token'] = 'SellwayBD_Steadfast_' . Str::random(24);
            }
        }
        ...
        if ($update_data->type === 'pathao') {
            Cache::forget('courier_pathao');
            Cache::forget('pathao_cities');
            Cache::forget('pathao_stores');
        }

        if ($update_data->type === 'redx') {
            Cache::forget('courier_redx');
            Cache::forget('redx_areas');
            Cache::forget('redx_pickup_stores');
        }

        if ($update_data->type === 'paperfly') {
            Cache::forget('courier_paperfly');
        }
